...
public class XenFloatingIsland2Feature extends Feature<NoneFeatureConfiguration> {
	public static final XenFloatingIsland2Feature FEATURE = (XenFloatingIsland2Feature) new XenFloatingIsland2Feature()
			.setRegistryName("undogo:xen_floating_island_2");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public XenFloatingIsland2Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("undogo", "xen.floating_island2"));
		if (template == null)
			return false;
