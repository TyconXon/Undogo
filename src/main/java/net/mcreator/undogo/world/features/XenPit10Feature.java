...
public class XenPit10Feature extends Feature<NoneFeatureConfiguration> {
	public static final XenPit10Feature FEATURE = (XenPit10Feature) new XenPit10Feature().setRegistryName("undogo:xen_pit_10");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public XenPit10Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("undogo", "xen.pit10"));
		if (template == null)
			return false;
