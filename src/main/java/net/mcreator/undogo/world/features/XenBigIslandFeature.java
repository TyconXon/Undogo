...
public class XenBigIslandFeature extends Feature<NoneFeatureConfiguration> {
	public static final XenBigIslandFeature FEATURE = (XenBigIslandFeature) new XenBigIslandFeature().setRegistryName("undogo:xen_big_island");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public XenBigIslandFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("undogo", "xen.bigisland1"));
		if (template == null)
			return false;
