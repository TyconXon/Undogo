...
public class XenPit7Feature extends Feature<NoneFeatureConfiguration> {
	public static final XenPit7Feature FEATURE = (XenPit7Feature) new XenPit7Feature().setRegistryName("undogo:xen_pit_7");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public XenPit7Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("undogo", "xen.pit7"));
		if (template == null)
			return false;
