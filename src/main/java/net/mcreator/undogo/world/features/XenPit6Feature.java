...
public class XenPit6Feature extends Feature<NoneFeatureConfiguration> {
	public static final XenPit6Feature FEATURE = (XenPit6Feature) new XenPit6Feature().setRegistryName("undogo:xen_pit_6");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public XenPit6Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("undogo", "xen.pit6"));
		if (template == null)
			return false;
