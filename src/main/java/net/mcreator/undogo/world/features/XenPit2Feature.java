...
public class XenPit2Feature extends Feature<NoneFeatureConfiguration> {
	public static final XenPit2Feature FEATURE = (XenPit2Feature) new XenPit2Feature().setRegistryName("undogo:xen_pit_2");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public XenPit2Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("undogo", "xen.pit2"));
		if (template == null)
			return false;
