...
public class XenSpireStructFeature extends Feature<NoneFeatureConfiguration> {
	public static final XenSpireStructFeature FEATURE = (XenSpireStructFeature) new XenSpireStructFeature()
			.setRegistryName("undogo:xen_spire_struct");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public XenSpireStructFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("undogo", "xen.spire1"));
		if (template == null)
			return false;
