...
public class XenPit3Feature extends Feature<NoneFeatureConfiguration> {
	public static final XenPit3Feature FEATURE = (XenPit3Feature) new XenPit3Feature().setRegistryName("undogo:xen_pit_3");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public XenPit3Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("undogo", "xen.pit3"));
		if (template == null)
			return false;
